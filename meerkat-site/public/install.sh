...
if [ "$UNINSTALL" -eq 1 ]; then
  ...
  # -L as well as -d: a current install puts a real bundle here, but one made by
  # an older installer left a symlink into the versions directory — and by the
  # time this runs that target is gone, so -d alone would miss it.
...
if [ -d "$TARGET/Meerkat.app" ]; then
  # A real copy in ~/Applications, not a link into the versions directory.
  # Spotlight and Launchpad do not index a symlinked bundle, and they index
  # nothing under ~/.meerkat at all — a dot-directory is invisible to them — so a
  # link leaves the app unfindable by name, which is how most people open an app.
  # The duplicated bundle is the price of "type Meerkat, press return".
  APP_DEST="$HOME/Applications/Meerkat.app"
  mkdir -p "$HOME/Applications"
  rm -rf "$APP_DEST"
  cp -R "$TARGET/Meerkat.app" "$APP_DEST"
  cat > "$BIN_DIR/meerkat-app" <<EOF
#!/bin/sh
# Generated by the Meerkat installer.
set -eu
"$BIN_DIR/meerkat-engine" start >/dev/null
exec open -a "$APP_DEST" "\$@"
EOF
